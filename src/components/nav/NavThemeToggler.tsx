import { switchModeHandler } from '../../utils/theme-handler';
...
const NavThemeToggler: React.FC<NavThemeTogglerProps> = ({theme, toggleTheme}) => {
	...
	return (
		<div className='absolute bottom-[3rem] flex justify-between items-center w-[10.4rem] h-[2.4rem] mt-8'>
			...
			<button className='relative w-[4.8rem] h-full bg-darkGray200 border-none outline-none rounded-[1.45rem] transition-colors duration-300 hover:bg-darkGray100 focus:bg-darkGray100'
			...
			<IconLightMode
				className={theme === 'light' ? 'icon-active' : ''}
			/>
		</div>
	);
};
